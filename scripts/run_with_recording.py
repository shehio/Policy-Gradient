...
def run_game_with_recording(game_name, max_episodes=None, output_dir="recordings", **kwargs):
    ...
    cmd = [sys.executable, "scripts/policy-gradient/pg_trainer.py", game_name, "--render"]
...
def main():
    parser = argparse.ArgumentParser(description="Run Policy Gradient training with recording")
    parser.add_argument("game", choices=['pong', 'breakout', 'pacman'], help="Game to train on")
    parser.add_argument("--max-episodes", type=int, help="Maximum episodes to run (not yet implemented)")
