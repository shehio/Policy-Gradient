...
class Game:
    def __init__(
        self, game_name: str, render: bool, pixels_count: int, episode_number: int
    ) -> None:
    ...
    def end_episode(self) -> None:
        ...
        if self.episode_number % 10 == 0 and len(self.recent_rewards) > 0:
            print(
                "Resetting env. Episode: %i, episode reward: %.1f, running mean(last 100 episodes): %.3f."
                % (self.episode_number, self.reward_sum, self.running_reward)
            )
    ...
    def get_frame_difference(self) -> np.ndarray:
        ...
        state = (
            ...
            else np.zeros(self.pixels_count)
        )
