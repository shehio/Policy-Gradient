...
def handle_breakout_fire(game: Game, info: Dict[str, Any]) -> None:
    ...
    if (
        hasattr(game, "previous_lives")
        and current_lives is not None
        and current_lives < game.previous_lives
    ):
        print(
            f"🔥 Life lost! Clicking fire. Lives: {game.previous_lives} -> {current_lives}"
        )
...
def handle_pacman_episode_end(game: Game, agent: Any) -> None:
    ...
    if game.episode_number % 10 == 0:
        recent_rewards = (
            agent.total_rewards[-10:]
            if len(agent.total_rewards) >= 10
            else agent.total_rewards
        )
        recent_avg = np.mean(recent_rewards) if recent_rewards else 0
        print(
            f"Episode {game.episode_number}: reward={game.reward_sum:.1f}, running mean={game.running_reward:.3f}, recent avg={recent_avg:.3f}"
        )
...
GAME_CONFIGS = {
    ...
    "pacman": GameConfig(
        ...
        preprocess_func=lambda game, prev_frame: preprocess_pacman_frame_color_aware_difference(
            game.observation, prev_frame
        ),
...
def get_game_config(game_name: str) -> GameConfig:
    ...
    if game_name not in GAME_CONFIGS:
        raise ValueError(
            f"Unknown game: {game_name}. Available games: {list(GAME_CONFIGS.keys())}"
        )
