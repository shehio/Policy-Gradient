...
class DQNGameConfig:
    ...
    def __init__(self, name: str, game_id: str, agent_class, 
...
def find_latest_dqn_model_episode(game_name: str, model_dir: str = "atari/scripts/dqn/models") -> int:
...
def get_dqn_game_config(game_name: str) -> DQNGameConfig:
    ...
    if game_name not in DQN_GAME_CONFIGS:
        raise ValueError(f"Unknown DQN game: {game_name}. Available games: {list(DQN_GAME_CONFIGS.keys())}")
...
def get_dqn_hyperparameters(game_name: str, config: Dict[str, Any]) -> HyperParameters:
    ...
    model_config = ModelConfig(
        ...
        load_model_from_file=not config.get('no_load_network', False) and game_config.model_config.load_model_from_file,
    ...
    learning_config = LearningConfig(
        gamma=game_config.learning_config.gamma,
        alpha=alpha_value
    )
