...
# Use 'CnnPolicy' for image-based observations like Atari MsPacman
model = PPO('CnnPolicy', env, verbose=1, tensorboard_log=log_directory)

save_callback = SaveEveryStepCallback(save_freq=100_000, save_path=model_directory, game_name='MsPacman')

timesteps = 100_000_000
epochs = 1000
model = train(model, timesteps, epochs, env_name, model_directory, callback=save_callback)
model.save(f'final_ppo_pacman_epochs_{epochs}_timesteps_{timesteps}') 