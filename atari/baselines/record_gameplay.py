...
def find_most_recent_model(game_name):
    ...
    if not model_timesteps:
        raise ValueError(f"Could not extract timesteps from model filenames for game '{game_name}'")
...
def main():
    ...
    parser.add_argument(
        "game_name",
        type=str,
        help="Name of the game (e.g., mspacman, pong, breakout)"
    )
