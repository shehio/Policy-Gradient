...
class AgentImproved:
    def __init__(self, environment, hyperparams):
        """
        Improved Agent initialization using hyperparameters passed from training script
        """
        ...
        self.epsilon_minimum = hyperparams.exploration.epsilon_minimum  # Minimum epsilon

        # Memory parameters from hyperparams
        self.memory = deque(maxlen=hyperparams.training.max_memory_len)

        # Create improved models using hyperparams
        self.online_model = DuelCNNImproved(h=self.target_h, w=self.target_w, output_size=self.action_size).to(hyperparams.environment.device)
        self.target_model = DuelCNNImproved(h=self.target_h, w=self.target_w, output_size=self.action_size).to(hyperparams.environment.device)
    ...
    def preProcess(self, image):
        ...
        frame = frame[self.crop_dim[0]:self.crop_dim[1], self.crop_dim[2]:self.crop_dim[3]]  # Cut from top
    ...
    def act(self, state):
        ...
        if act_protocol == 'Explore':
            ...
            with torch.no_grad():
                state = torch.tensor(state, dtype=torch.float, device=self.hyperparams.environment.device).unsqueeze(0)
                q_values = self.online_model.forward(state)  # (1, action_size)
                action = torch.argmax(q_values).item()  # Returns the indices of the maximum value
    ...
    def train(self):
        ...
        state, action, reward, next_state, done = zip(*random.sample(self.memory, self.hyperparams.training.batch_size))
        ...
        state = torch.tensor(state, dtype=torch.float, device=self.hyperparams.environment.device)
        next_state = torch.tensor(next_state, dtype=torch.float, device=self.hyperparams.environment.device)
        action = torch.tensor(action, dtype=torch.long, device=self.hyperparams.environment.device)
        reward = torch.tensor(reward, dtype=torch.float, device=self.hyperparams.environment.device)
        done = torch.tensor(done, dtype=torch.float, device=self.hyperparams.environment.device)
        ...
        torch.nn.utils.clip_grad_norm_(self.online_model.parameters(), max_norm=10.0)
        
        self.optimizer.step()
        
        # Update target network every 10,000 steps (as per description)
