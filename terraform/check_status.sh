...
if [ "$PUBLIC_IP" != "None" ] && [ -n "$PUBLIC_IP" ]; then
    ...
    echo "ssh -i generic.pem ubuntu@$PUBLIC_IP"
    echo ""
    echo "To check the user data logs:"
    echo "ssh -i generic.pem ubuntu@$PUBLIC_IP 'sudo cat /var/log/user-data.log'"
    echo ""
    echo "To check if the training is running:"
    echo "ssh -i generic.pem ubuntu@$PUBLIC_IP 'ps aux | grep python'"
    echo ""
